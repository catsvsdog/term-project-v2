...
def onAppStart(app):
    ...
    top1 = Top("bf", {"green"}, {"summer"}, "baggy", "IMG_1469")
    top2 = Top("bm",{ "blue"}, {"summer"}, "tight", "IMG_1479")
    top3 = Top("Pacsun", {"red"}, {"summer", "spring"}, "tight", "IMG_1473")
    top4 = Top("bm", {"white"}, {"summer", "spring"}, "tight", "IMG_1470")
    ...
    # app.final = []
    app.likedOutfits = []
    #app.userPref = {"color": set()}

    for item in loadImage("processed"):
...
def redrawAll(app):
  ...
  # if app.regen:
  #   outfits_size = len(app.displayed_outfits)
  #   loop_range = loop_range = min(outfits_size, 4)

  #   for i in range(loop_range): # loop from 0 to 4
  #     outfit = pickOutfitOnUserPreference(app.displayed_outfits, app.userPrefColors)
  #     if not outfit:
  #         print('Warning: outfit is empty, nothing to display!')
  #         return None, None
  #     topItem = None
  ...
  #       # add displayed outfit into display outfit list
  #       app.preferred_outfits.append(outfit)
  #       print('size of good outfits = ', len(app.my_good_outfits))
  #       print('displayed = ', len(app.displayed_outfits))

  #   #return app.displayed_outfits, preferred_outfits
  # else:
  ...
  for i in range(loop_range):  # loop from 0 to 4
      ...
      # outfits_size = len(app.my_good_outfits)
      # loop_range = min(outfits_size, 4)
      # app.my_good_outfits, app.displayed_outfits = display4RandomOutfits(app.my_good_outfits, app.displayed_outfits)
      # print(Item.instances)
    ...
    if app.regen == False:
      for i in range(len(app.xPos)):
        ...
        if (mouseX >= j - 75 and mouseX <= j + 75) and (mouseY >= 525 and mouseY <= 575):
            if app.displayed_outfits[i] not in app.likedOutfits:
                app.likedOutfits.append(app.displayed_outfits[i])
      ...
      if (mouseX >= 675 and mouseX <= 825) and (mouseY >= 575 and mouseY <= 625):
        ...
        app.regen = True
        app.xPos.clear()
          # if app.userPrefColors != dict():
          #    app.hasPref = True
# def redrawAll(app):
#     app.xPos.clear()

#     # Check if there are any outfits to display initially
#     if not app.regen:  
#         # Ensure 4 outfits are drawn initially if there are items in displayed_outfits
#         outfits_size = len(app.displayed_outfits)
#         print(f"Outfits to display: {outfits_size}")

#         if outfits_size == 0:
#             print("No outfits to display. Please check app.displayed_outfits.")
#             return  # Exit early if no outfits are available
...
#                 # Debugging: Print position and item details
#                 print(f"Drawing outfit {i}: x={x}, y={y}, top={topItem.name}, bottom={bottomItem.name}")
...
#                 # Add displayed outfit into preferred outfits
#                 app.displayed_outfits.remove(outfit)
#                 app.preferred_outfits.append(outfit)
...
#         drawLabel("regenerate", 750, 600, size=17, font="Lora")
#     else:
#         # When regen is True, display based on preferences or random outfits
#         outfits_size = len(app.displayed_outfits)
#         loop_range = min(outfits_size, 4)
...
# def onMousePress(app, mouseX, mouseY):
#   # Handle "add to outfits" button click
#   for i in range(len(app.xPos)):
#       x = app.xPos[i]
#       if (mouseX >= x - 75 and mouseX <= x + 75) and (mouseY >= 525 and mouseY <= 575):
#           if app.displayed_outfits[i] not in app.likedOutfits:
#               app.likedOutfits.append(app.displayed_outfits[i])
#   print("Liked outfits:", app.likedOutfits)
#   addUserPref(app.likedOutfits, app.userPrefColors)
