...
class Top(Item):
    def __init__(self, image, name, color, season, type, serialNum = 0):
        self.super()
        #super().__init__(image, name, None, None, color, season, type, 'top', serialNum)
        self.name = name
        self.color = color
        self.season = season
...
class Bottom(Item):
    def __init__(self, image, name, color, season, type, serialNum = 0):
        #super().__init__(image, name, None, None, color, season, type, 'bottom', serialNum)
        self.name = name
        self.color = color
        self.season = season
        self.type = type
...
class Outfit:
    ...
    def isGoodOutfit(self):
        bot = list(self.bottom.color)
        top = list(self.top.color)
            ...
            for colorTop in top:
                ...
                if colorTop in colorsThatGoWith: 
                    ...
                    if colorBottom in colorsThatGoWith[colorTop]:
                        if self.bottom.type in types.get(self.top.type):  # Safe dictionary access
                            return True
        return False 
